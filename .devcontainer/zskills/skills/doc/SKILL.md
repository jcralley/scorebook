...
2. **Write the entry** following this structure:
   ...
   - **Body:** pick the format that fits:
     - _User-facing feature:_ opening paragraph → how to start (numbered
       steps) → how it works (bullets) → optional architecture note
     - _Block additions:_ "Expanded from X to **Y blocks**" → bullet list
       of new blocks → new examples → updated counts
     - _QE/bugfix batch:_ scope summary → notable fixes → deferred count
     - _Multi-feature drop:_ bold sub-headers → bullet lists under each
