...
4. **Unconfigured hook placeholders?**
   ...
   exists, the hook will block the eventual `git commit` with _"Test
   infrastructure detected but FULL_TEST_CMD not configured"_ — so
   catching it at preflight just prevents wasted work. Its job is to
   notice when a project _has_ test infrastructure but the hook doesn't
   yet know about it.
   ...
   - **Placeholders found AND test infra exists** — where test infra means
     ...
     **STOP.** Hook placeholders have not been configured — run
     ...
     enforcement matters. Report: _"hook-placeholder gate tripped:
     placeholders present AND test infra detected — stopping."_
   - **Placeholders found, no test infra**: gate silent, proceed. This is
     ...
     same phase, so subsequent runs have real enforcement. Report: _"gate
...
4. **Mark the phase as 🟡 In Progress and commit** (mode-conditional location):
   ...
   feature branch _before_ push so it's captured in the squash). If
